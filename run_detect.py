...
#   for f in listdir(path_ts_file):
  ...
  task_colnames = ['ST','ET','JI', 'TI','MI', 'S', 'SN', 'TSN', 'MCPU', 'ACPU', 'MM', 'AM']
  ...
  task_cols = ['Latency','MCU', 'CMU', 'AMU', 'UPC', 'TPC', 'MAXMU', 'MIO',
            'MDK', 'MAXCPU', 'MAXIO', 'CPI', 'MAI', 'SCPU', 'EV', 'FL']
  task_fields = ['ST','ET','JI', 'TI','MI', 'SN', 'TSN', 'MCPU', 'ACPU', 'MM', 'AM']
  task_cols = ['ST','ET','JI', 'TI','MI', 'SN', 'TSN', 'MCPU', 'ACPU', 'MM', 'AM']
  ## Get cumulative time series data
  list_task = [] 
  list_tp = []  ## list of total period
  list_task_compact = []  ## list of last row
  for i in range(job_rawsize):
    task = pd.read_csv('{}/{}/{}'.format(path_ts,jobid,files_task[i]), header=None,
                       names=task_colnames, index_col=False)
    task = task.drop(columns=['S'])
  ...
  for dd in list_task_nn:
      if dd.shape[0] < ts_init_size:
        ...
        #   print(type(dd), type(df2))
          list_task_norm.append(pd.concat([dd,df2], ignore_index=True))
      else:
